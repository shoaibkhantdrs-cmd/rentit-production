...
class InMemoryUserRepository {
    ...
    async search(filters, page, pageSize) {
        ...
        if (filters.query) {
            ...
            candidates = candidates.filter((u) => u.name.toLowerCase().includes(needle) ||
        ...
        let withRoles = await Promise.all(candidates.map(async (user) => ({
            user,
            roles: this.userRoleRepo ? await this.userRoleRepo.listRoleNamesForUser(user.id) : [],
        })));
        if (filters.role) {
            withRoles = withRoles.filter((row) => row.roles.includes(filters.role));
        }
    ...
    async countCreatedBetween(from, to) {
        return Array.from(this.users.values()).filter((u) => u.createdAt.getTime() >= from.getTime() && u.createdAt.getTime() < to.getTime()).length;
    }
}
