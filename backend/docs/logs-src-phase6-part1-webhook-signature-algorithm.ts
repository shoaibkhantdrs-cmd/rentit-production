...
function razorpayVerify(rawBody: Buffer, signatureHeader: string | undefined, secret: string): boolean {
...
function stripeVerify(rawBody: Buffer, signatureHeader: string | undefined, secret: string, tolerance = 300): boolean {
  if (!signatureHeader) return false;
...
const v1 = createHmac("sha256", stripeSecret).update(signedPayload).digest("hex");
