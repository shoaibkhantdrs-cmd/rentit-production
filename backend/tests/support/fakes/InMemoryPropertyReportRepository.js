...
exports.InMemoryPropertyReportRepository = void 0;
const ids_1 = require("./ids");
class InMemoryPropertyReportRepository {
    reports = [];
    async create(input) {
        const now = new Date();
        const report = {
    ...
    async existsForUserAndProperty(propertyId, userId) {
    ...
    async updateStatus(id, status, reviewedBy) {
        ...
        if (!existing)
            throw new Error(`Property report ${id} not found`);
...
exports.InMemoryPropertyReportRepository = InMemoryPropertyReportRepository;
