...
exports.InMemoryPropertyRepository = void 0;
...
class InMemoryPropertyRepository {
    ...
    async findByOwner(ownerId, page, pageSize) {
    ...
    async findManyByIds(ids) {
        ...
        return ids
            ...
            .filter((p) => Boolean(p) && !p.deletedAt);
    }
    async search(options) {
        const { filters } = options;
        let candidates = Array.from(this.properties.values()).filter((p) => !p.deletedAt && p.status === "published");
        if (filters.categoryId)
            candidates = candidates.filter((p) => p.categoryId === filters.categoryId);
        if (filters.propertyType)
            candidates = candidates.filter((p) => p.propertyType === filters.propertyType);
        if (filters.rentMin !== undefined)
            candidates = candidates.filter((p) => p.rentAmount >= filters.rentMin);
        if (filters.rentMax !== undefined)
            candidates = candidates.filter((p) => p.rentAmount <= filters.rentMax);
        ...
        if (filters.areaMin !== undefined)
            candidates = candidates.filter((p) => p.areaSqft >= filters.areaMin);
        if (filters.areaMax !== undefined)
            candidates = candidates.filter((p) => p.areaSqft <= filters.areaMax);
        if (filters.furnished)
            candidates = candidates.filter((p) => p.furnishedStatus === filters.furnished);
        ...
        let withDistance = filtered.map((row) => ({ ...row, distanceKm: null }));
        if (filters.latitude !== undefined &&
            ...
            withDistance = withDistance
                ...
                .map((row) => ({
                ...row,
                distanceKm: (0, haversine_1.haversineDistanceKm)(filters.latitude, filters.longitude, row.location.latitude, row.location.longitude),
            }))
    ...
    async adminSearch(options) {
        ...
        if (filters.categoryId)
            candidates = candidates.filter((p) => p.categoryId === filters.categoryId);
        if (filters.ownerId)
            candidates = candidates.filter((p) => p.ownerId === filters.ownerId);
        ...
        if (filters.createdTo)
            candidates = candidates.filter((p) => p.createdAt <= filters.createdTo);
        if (filters.city) {
            const needle = filters.city.toLowerCase();
            candidates = candidates.filter((p) => this.locationRepo.byPropertyId.get(p.id)?.city.toLowerCase().includes(needle));
        }
...
exports.InMemoryPropertyRepository = InMemoryPropertyRepository;
