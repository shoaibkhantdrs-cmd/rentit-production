...
test("End-to-end: a saved search fires, then the seeker views, chats, and WhatsApps the owner", async () => {
  ...
  assert.equal(notifications.items[0].data?.propertyId, property.id);
  ...
  assert.equal(recentlyViewed.items[0]?.id, property.id);
  ...
  assert.equal(ownerUnread.unreadCount, 1);
  ...
  assert.equal(ownerUnreadAfter.unreadCount, 0);
  ...
  assert.equal(container.whatsAppService.sent[0].to, "+919812300001");
  ...
  assert.ok(recommendations.items.some((i) => i.id === secondListing.id));
  ...
  assert.ok(refreshedSearch?.lastNotifiedAt);
});
